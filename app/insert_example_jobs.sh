...
APIURL="http://127.0.0.1:8098/api"
...
STATECHNAGESUCCESSJOBGUID=$(curl -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' -d "${JSON}" ${APIURL}'/jobs/' | \
    python3 -c "import sys, json; print(json.load(sys.stdin)['guid'])")
JSON="{    \"enabled\": false,    \"name\": \"SC Fail\",    \"repetitionInterval\": \"\",    \"command\": \"ls -la\"  }"
STATECHNAGEFAILJOBGUID=$(curl -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' -d "${JSON}" ${APIURL}'/jobs/' | \
    python3 -c "import sys, json; print(json.load(sys.stdin)['guid'])")
JSON="{    \"enabled\": false,    \"name\": \"SC Unknown\",    \"repetitionInterval\": \"\",    \"command\": \"ls -la\"  }"
STATECHNAGEUNKNOWNJOBGUID=$(curl -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' -d "${JSON}" ${APIURL}'/jobs/' | \
    python3 -c "import sys, json; print(json.load(sys.stdin)['guid'])")
...
do
  ...
  curl --fail -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' -d "${JSON}" ${APIURL}'/jobs/'
  RES=$?
  echo $RES
  if [[ ${RES} -ne 0 ]]; then
...
JWEGUID=$(curl -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' -d "${JSON}" ${APIURL}'/jobs/' | \
    python3 -c "import sys, json; print(json.load(sys.stdin)['guid'])")
