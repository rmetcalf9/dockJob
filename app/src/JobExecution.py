...
class JobExecutionClass():
  ...
  def execute(self, executor):
    self.stage = 'Running'
    self.dateStarted = datetime.datetime.now(pytz.timezone("UTC"))
    try:
      ...
      self.stage = 'Timeout'
      self.dateCompleted = datetime.datetime.now(pytz.timezone("UTC"))
      return
    self.resultReturnCode = executionResult.returncode
    self.resultSTDOUT = executionResult.stdout
